...
trading at worse prices than they expect. this is called execution price uncertainty. 


## Limit Orders 

A limit order is an isntrucction to trade at the best price available but only 
...
In continously trading markets, a broker (or an exchange) will attempt to trade 
a newly submitted limit order as soon as it arrives. if no trader is 
immediately willing to take the opposite side at an acceptable price, the 
order will not trade. Instead, it will stand as an offer to trade until 
someone is willing to trade at its limit price, until it expires, or until 
the trader who submiteted it cancels it. 
...
maintin the limit order book. 

The probability that a limit order will trade depends on its limit price. If the limit 
price of a buy order is too low, the order will not trade. Likewise, if a sell 
limit price is too high, the order will not trade. 

Buy limit orders with high prices and sell limit orderxs with low prices are aggressively 
priced. Aggressively priced limit orders are the easiest limit orders to fill. 

* Limit Price Placement 

Traders classify limit orders by where they place their limit prices relative to 
the market. The market is the range of prices bounded above by the best 
offer (lower price) and below by the best bid. 

A marketable limit order is an order that the broker (or exchange) can execute 
immediately when a trader submits it.

Limit orders that stand behind the best bid or offer are behind the market. Such 
orders are away from the market. A buy order is behind the market if its limit 
price is less than the best bid. A sell order is behind the market if its 
limit price is above the best offer. 

* Standing Limit Orders Are Trading Options that Offer Liquidity 

Tradrers who submit standing limit orders offer liquidity to other traders. 
Their limit orders give others the ability to trade when they want to trade. 
Since traders can choose whetehr they want to trade with a standing limit 
order, standing limit orders are option to trade. 
...
after price falls to the stop price. Orders with stop instructions are 
called stop orders. 

Traders most common:ly use stop orders to stop their losses when prices move 
against their positions.

* STOP Orders and Limit Orders 

There can sometimes exist a confusion between stop orders and limit orders because 
both specify price conditions. The difference lies in the purpose of the specified 
price. A stop instruction provides for the activation of an order when the market 
price reaches or passes a specified stop price.


* Stop Orders and Liquidity 

Stop orders accelerate price changes. Price often change because traders on one side 
of the market demand more liquidity than is available. When these price changes 
activate stop orders, the stop order unfortunately contribute the one-sided demands 
for liquidity

## SUmmary 

Traders use orders to communicate their intentions to the brokers and exchanges that 
arrange their trades. The most important and most common order types are market orders 
and limit orders. Traders use market orders when they want to trade immediately at the 
best price the market will bear. They use limit orders when they want to place limits 
on the prices at which their are willing to trade. 

Orders differ by their associated uncertainties. Traders who use market orders are uncertiain 
aboput the prices at which they will trade. Conversly, traders who use limit orders are 
uncertain about whether they will trade. 
...
- Large market orders can have a substantial and unpredictable price impacts 
